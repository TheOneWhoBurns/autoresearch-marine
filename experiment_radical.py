...
def main():
    ...
    for i in range(n_segs):
        ...
        # 4 classes — NOAA-based (unit-agnostic) + simple acoustic split
        if noaa_max >= 0.7:
            eco_labels.append("whale")
        elif noaa_max >= 0.4:
            eco_labels.append("possible_whale")
        elif noaa_mean >= 0.1:
            eco_labels.append("faint_bio")  # some biological signal detected
        else:
            eco_labels.append("background")
    ...
    for held_out_unit in unique_units:
        ...
        class EcoClassifier(nn.Module):
            def __init__(self, n_mels, time_dim, n_classes):
                super().__init__()
                self.conv = nn.Sequential(
                    nn.Conv2d(1, 32, 3, padding=1), nn.BatchNorm2d(32), nn.ReLU(), nn.MaxPool2d(4),
                    ResBlock(32),
                    nn.Conv2d(32, 64, 3, padding=1), nn.BatchNorm2d(64), nn.ReLU(), nn.MaxPool2d(4),
                    ResBlock(64),
